...
impl Rule for RepeatedKeys {
    fn validate(&self, node: &SyntaxElement, _sess: &SessionInfo) -> Option<Report> {
        if_chain! {
            ...
            let occurrences = parent_attr_set.entries().filter_map(|entry| {
            match entry {
                ...
                if kv_scrutinee_ident.to_string() == first_component_ident.to_string() {
                    Some((
                            kv_scrutinee.attrpath()?.syntax().text_range(),
            ...
            then {
                ...
                let third_message = {
                    ...
                    message.push_str(&format!(" Try `{} = {{ {}=...; {}=...; {}=...; }}` instead.", first_component_ident.to_string(), first_subkey, second_subkey, third_subkey));
                    message
                };

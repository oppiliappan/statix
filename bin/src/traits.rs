...
fn write_stderr<T: Write>(
    ...
    let range = |at: TextRange| at.start().into()..=(at.end() - TextSize::from(1)).into();
    let src_id = path.to_str().unwrap_or("<unknown>");
    lint_result.reports.iter().try_for_each(|report| {
        let offset = report
            .diagnostics
            .iter()
        ...
        let prefix = match report_kind {
            CliReportKind::Error => "E",
            CliReportKind::Warning => "W",
            CliReportKind::Advice => "A",
            CliReportKind::Custom(_, _color) => "C",
        };
        CliReport::build(report_kind, (src_id, offset..=src_id.len()))
            .with_config(
                CliConfig::default()
                    .with_cross_gap(true)
            ...
            .with_code(format!("{prefix}{:02}", report.code))
            .with_labels(report.diagnostics.iter().map(|diagnostic| {
